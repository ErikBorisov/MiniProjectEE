...
public class CommentManager {
    ...
    @SneakyThrows
    public List<Comment> add(Comment comment, Article article) {
        List<Comment> comments = new ArrayList<>();
        PreparedStatement preparedStatement = connection.prepareStatement("select * from comments where article_id = ?");
        preparedStatement.setInt(1, article.getId());
        ResultSet resultSet = preparedStatement.executeQuery();
        while (resultSet.next()) {
            int id = resultSet.getInt("id");
            String content = resultSet.getString("content");
            int user_id = resultSet.getInt("author_id");
    ...
    @SneakyThrows
    public List<Comment> userArticleComment(Article article) {
        List<Comment> comments = new ArrayList<>();
        ...
        while (resultSet.next()) {
            ...
            comments.add(Comment.builder()
                    ...
                    .author(User.builder()
                            ...
                            .surname(resultSet.getString("authorSurname"))
                            .build())
                    .build());
        }
        return comments;
